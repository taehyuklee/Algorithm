...
* <b>사람의 이동</b>
	...
	* 매 초 사람도 한 칸씩 움직인다.
        ...
        * 사람은 울타리 밖으로 움직이지 못한다.
        * 현재 소와의 거리보다 더 멀어지게 움직일수는 없다.
  <br>
  
* 박치기 (두 가지 케이스) <br>

   1. case) 소가 사람한테 박을때 (소가 박은 방향으로 사람이 C거리만큼 날아간다) <br>
   2. case) 사람이 소한테 박을때 (사람이 소한테 돌진한 반대 방향으로 D거리만큼 날아간다)
      - 날아간 사람은 K초에 날아간 시점에서 K+1초까지 기절해 있다. 기절한 사람은 기절한 동안에 움직일수 없다.
      - 그리고 소에게 박치기 당하면 울타리 밖으로 나갈수 있으며, 나간 경우에는 다시는 참여하지 못한다.

  사람이 포물선으로 날아가기때문에 날아가는 동안 그 경로에 있는 사람들은 무시하고 날아갈수 있다. <br>
  하지만, 도착지에 사람이 있으면 <b> 상호작용 </b>을 하게 된다.

  * 상호작용이란?
    1. 사람이 날아가면 그 자리에 있는 사람은 날아온 사람이 가지고 있는 방향으로 1거리 만큼 밀려난다.
    2. 위 상호작용은 연쇄적으로 일어난다. <br>
    	(예를 들어 사람이 밀려나서 뒤로 밀려난 사람이 1칸 밀려났을때 그 자리에도 사람이 있다면 상호작용이 또 일어난다)
    3. 상호작용에 의해 사람은 울타리 밖으로 나갈수 있다.
 <br>
 
* 1초마다 장외나지 않은 모든 사람들에게 1점씩 부여한다.

------------------
위의 문제에 대한 고찰 
구현량이 상당히 많았고, 결국 테스트케이스를 모두 맞추지 못했다. 박치기 부분은 제대로 구현했다.
끝나고 친구랑 얘기하다 알게 된 부분은 사람의 이동, 소의 이동을 너무 힘들게 구현했다. 진짜 BFS를 쓸 필요가 없었다. 근데 생각이 안나서 BFS를 구현하고 모든 케이스를 리스팅 했다.
사람의 움직임도 마찬가지고 -> 그래서 진짜 너무 힘들게 구현했고 (진짜 이 부분 바로 구현하고 끝냈으면 20분이면 다 하고 넘어갈 것을...) 실수도 있었고 진짜 말도 안되는 생각을 해서.. 박치기에서 오래 걸려야 하는데;;
