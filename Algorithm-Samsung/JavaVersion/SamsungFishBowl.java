...
public class Main {
	...
	static int stInx_c, endInx_c, stInx_r, endInx_r, answer;
	...
	public static void solution() {
		...
		while(true) {
			...
			//6. 반 접어서 180도 공중부양. (두번 접기)
			int[][] halfed2D= half180();
			
			//7. board에 insert
			insert2Dboard(halfed2D);
			...
			//10. 최대와 최소 차이
			copyArray(straight, straight2);
			
			answer+=1;
			Arrays.sort(straight2);
			
			if(straight2[straight2.length-1] - straight2[0] <=K) {
				break;
			}
		}
	...
	public static void insert2Dboard(int[][] halfed2D) {
		//물고기 control format에 맞춰줘야 한다.
		board = new int[N][N];
		board2 = new int[N][N];
		for(int i=halfed2D.length-1; i>=0; i--) {
			for(int j=0; j<halfed2D[0].length; j++) {
				board[i][j] = halfed2D[i][j];
			}
		}
	}
	...
	public static int[][] half180() {
		...
		for(int i=0; i< 2; i++) {
			for(int j=halAndHalf; j<rotatedBoard[0].length; j++) {
				...
				rotate2D[rotate2D.length-1 -i][j-halAndHalf] = rotatedBoard[rotatedBoard.length-1 -i][j];
			}
		}
		...
		for(int i=0; i< 2; i++) {
			for(int j=0; j<halAndHalf; j++) {
				//180도 회전
				rotate2D[(rotate2D.length-1)-2-i][N/4-1-j] = rotatedBoard[i][j];
			}
		}
